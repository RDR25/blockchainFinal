...
/**
 * JointCollectorDashboard.js
 * ------------------------------
 * File Name: JointCollectorDashboard
 * Description: Styled dashboard with full UI fixes as per permanent color scheme and layout.
...
function JointCollectorDashboard() {
 const navigate = useNavigate();
     ...
     fetch(`${backendUrl}/api/applicationdetails/sent?all=true`)
       ...
       .then(data => {
         if (data.applications) {
           // Applications sent to Joint Collector
           const sentApps = data.applications.filter(app => app.status === "Sent to Joint Collector");
           ...
           // Applications with status "Rejected by Joint Collector"
           const rejectedApps = data.applications.filter(app => app.status === "Rejected by Joint Collector");
   ...
   // Fetch notes for the logged in official (type "joint_collector")
   const fetchUserNotes = () => {
     if (official && official.officialid) {
       fetch(`${backendUrl}/api/notes?type=joint_collector&username=${official.officialid}`)
         .then(response => response.json())
         .then(data => {
           if (data.success) {
   ...
   // Add a note with type "joint_collector"
   const handleAddNote = () => {
  ...
  Joint Collector Dashboard
          ...
          <Button onClick={handleLogout} variant="contained" sx={{ bgcolor: '#434343', color: 'white', '&:hover': { bgcolor: '#000000' } }}>
        ...
        <Box sx={{ display: 'flex', flex: 1 }}>
          <Box sx={{ width: 80, background: 'linear-gradient(to bottom, #868f96, #596164)', color: 'white', display: 'flex', flexDirection: 'column', alignItems: 'center', py: 3 }}>
            {[{ icon: 'globe', path: '/jointcollectorhome' }, { icon: 'sign-out-alt', action: handleLogout }].map((item, i) => (
          ...
          <Box flex={1} p={3} display="grid" gridTemplateColumns="repeat(2, 1fr)" gap={3}>
            <Paper elevation={3} sx={{ p: 2, background: 'linear-gradient(145deg, #cfd9df, #e2ebf0)' }}>
              <Typography variant="h6" fontWeight="bold" sx={{ mb: 2 }}><i className="fas fa-hourglass-half" style={{ marginRight: 8 }}></i>Pending Applications</Typography>
              <Box display="flex" gap={2}>
                <Paper sx={{ flex: 1, p: 2, background: 'linear-gradient(145deg, #cfd9df, #e2ebf0)' }}><Typography fontWeight={600}>Total Pending Applications</Typography><Typography variant="h4">{pendingCount}</Typography></Paper>
                <Paper sx={{ flex: 1, p: 2, background: 'linear-gradient(145deg, #cfd9df, #e2ebf0)' }}><Typography fontWeight={600}>Received in Last 5 Days</Typography><Typography variant="h4">{recentCount}</Typography></Paper>
              </Box>
            </Paper>

            <Paper elevation={3} sx={{ p: 2, position: 'relative', background: 'linear-gradient(145deg, #cfd9df, #e2ebf0)' }}>
              ...
              {userNotes.length === 0 && <Typography>No notes found</Typography>}
              {userNotes.map(note => (
            ...
            <Paper elevation={3} sx={{ p: 2, background: 'linear-gradient(145deg, #cfd9df, #e2ebf0)' }}>
              <Typography variant="h6" fontWeight="bold"><i className="fas fa-times-circle" style={{ marginRight: 8 }}></i>Rejected Applications</Typography>
              <Box display="flex" gap={2}>
                <Paper sx={{ flex: 1, p: 2, background: 'linear-gradient(145deg, #cfd9df, #e2ebf0)' }}><Typography fontWeight={600}>Rejected by Joint Collector</Typography><Typography variant="h4">{rejectedCount}</Typography></Paper>
                <Paper sx={{ flex: 1, p: 2, background: 'linear-gradient(145deg, #cfd9df, #e2ebf0)' }}><Typography fontWeight={600}>Rejected in Last 2 Days</Typography><Typography variant="h4">{rejectedRecentCount}</Typography></Paper>
              </Box>
            </Paper>
...
export default JointCollectorDashboard;
