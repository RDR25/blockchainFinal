...
function VroHome() {
  ...
  const [chatRole, setChatRole] = useState("Revenue Inspector");
  ...
  const fetchPendingApplications = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const pending = (data.applications || []).filter(app => (app.status === "Sent to VRO"));
  ...
  const fetchChatApplications = async () => {
    if (!official?.officialid) return;
    try {
      const response = await fetch(`/api/chats/vrofficer`);
      const data = await response.json();
      if (response.ok) {
  ...
  const fetchSentApplications = async () => {
    ...
    try {
      const response = await fetch(`/api/applicationdetails/sent`);
      const data = await response.json();
      if (response.ok) setSentApplications((data.applications || []).filter(app => (app.status !== "Sent to ITDA" && app.status !=="created" && app.status!="Pending with User" && app.status != "Sent to Super Intendent" && app.status != "Rejected by Super Intendent" && app.status != "Sent to Project Officer 1" && app.status != "Rejected by Project Officer" && app.status != "Sent to MRO" && app.status != "Rejected by MRO" && app.status != "Sent to Surveyor" && app.status != "Rejected by Surveyor" && app.status != "Sent to Revenue Inspector" && app.status != "Rejected by Revenue Inspector" && app.status != "Sent to VRO")));
  ...
  const handleModalAction = async (actionType) => {
    ...
    if (actionType === "proceed") {
      newStatus = "Sent to RDO";
    } else if (actionType === "deny") {
      if (!denialReason.trim()) {
        alert("Please provide a reason for denial.");
        return;
      }
      newStatus = "Rejected by VRO";
      reason = denialReason;
    }
    try {
  ...
  const loadChatHistory = async (role, app = selectedChatApp) => {
    if (!app) return;
    try {
      const response = await fetch(`/api/chats/vro?appId=${app?._id || ""}&receiptid=${app?.receiptid || ""}&role=${role}`);
      const data = await response.json();
      if (response.ok) {
...
` }} />
      ...
      <div className="blue-line">
        ...
        <div className="header-title"> VRO Dashboard</div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: '20px', marginTop: '20px' }}>
                <div style={{
                  ...
                  <p><i className="fas fa-user-tag"></i> Role: VRO</p>
    ...
    {chatMessages.length === 0 ? (
      ...
      chatMessages.map((msg, idx) => (
        <div key={idx} style={{ marginBottom: "10px" }}>
          <strong>{msg.from === "vro" ? <i className="fas fa-user"></i> : <i className="fas fa-user-tie"></i>} {msg.from === "vro" ? "You" : chatRole}:</strong> {msg.message}
        </div>
      ))
    )}
  </div>
