...
function ClerkHome() {
  ...
  // File upload & application form states
  ...
  useEffect(() => {
    const storedClerk = localStorage.getItem("official");
    if (storedClerk) setClerk(JSON.parse(storedClerk));
  }, []);
  ...
  const fetchPendingApplications = async () => {
    if (!clerk?.officialid) return;
    try {
      const response = await fetch(`/api/applicationdetails/sent`);
      const data = await response.json();
      if (response.ok) {
        const pending = (data.applications || []).filter(app => app.status === "Sent to ITDA");
  ...
  const fetchChatApplications = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/chats/clerk`);
      const data = await response.json();
      if (response.ok) {
  ...
  const fetchSentApplications = async () => {
    ...
    try {
      const response = await fetch(`/api/applicationdetails/sent`);
      const data = await response.json();
      if (response.ok) setSentApplications((data.applications || []).filter(app => (app.status !== "Sent to ITDA" && app.status !=="created")));
  ...
  const handleModalAction = async (actionType) => {
    ...
    if (actionType === "proceed") {
      newStatus = "Sent to Super Intendent";
    } else if (actionType === "deny") {
      if (!denialReason.trim()) {
        alert("Please provide a reason for denial.");
        return;
      }
      newStatus = "Pending with User";
      reason = denialReason;
    }
    try {
  ...
  const loadChatHistory = async (role, app = selectedChatApp) => {
    if (!app) return;
    try {
      const response = await fetch(`/api/chats?appId=${app?._id || ""}&receiptid=${app?.receiptid || ""}&role=${role}`);
      const data = await response.json();
      if (response.ok) {
...
.button-close:hover {
 ...
 background: white; color: #000000; border: 1px solid #000000;}
      ...
      <div className="blue-line">
        ...
        <div className="header-title"> Clerk Dashboard</div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: '20px', marginTop: '20px' }}>
                <div style={{
                  ...
                  <p><i className="fas fa-user-tag"></i> Role: Clerk</p>
          ...
          {activePage==="inbox" && (
            <div>
              ...
              {chatApplications.length === 0 ? (
                <p><i className="fas fa-exclamation-circle"></i> No chat records found for Clerk.</p>
              ) : (
          ...
          <div className="modal-container">
            ...
            <div className="modal-body">
              ...
              <div className="modal-actions">
                <button className="button" onClick={() => handleModalAction("proceed")}>
              ...
              <div style={{ marginTop: "20px" }}>
                ...
                />
                <button className="button" onClick={() => handleModalAction("deny")}>
