...
function DistrictCollectorHome() {
  ...
  const [chatRole, setChatRole] = useState("Joint Collector");
  ...
  const fetchPendingApplications = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const pending = (data.applications || []).filter(app => (app.status === "Sent to District Collector"));
  ...
  const fetchChatApplications = async () => {
    if (!official?.officialid) return;
    try {
      const response = await fetch(`/api/chats/districtcollector`);
      const data = await response.json();
      if (response.ok) {
  ...
  const handleModalAction = async (actionType) => {
    ...
    if (actionType === "proceed") {
      newStatus = "Sent to Ministry of Welfare";
    } else if (actionType === "deny") {
      if (!denialReason.trim()) {
        alert("Please provide a reason for denial.");
        return;
      }
      newStatus = "Rejected by District Collector";
      reason = denialReason;
    }
    try {
  ...
  const loadChatHistory = async (role, app = selectedChatApp) => {
    if (!app) return;
    try {
      const response = await fetch(`/api/chats/dc?appId=${app?._id || ""}&receiptid=${app?.receiptid || ""}&role=${role}`);
      const data = await response.json();
      if (response.ok) {
...
.button-takeaction {
  background: linear-gradient(135deg,rgb(13, 13, 14),rgb(71, 73, 75));
}
.button-proceed {
...
.button-close:hover {
  ...
  box-shadow: 0 8px 25px rgba(220,53,69,0.4);
      ...
      <div className="blue-line">
        ...
        <div className="header-title"> District Collector Dashboard</div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: '20px', marginTop: '20px' }}>
                <div style={{
                  ...
                  <p><i className="fas fa-user-tag"></i> Role: District Collector</p>
    ...
    {chatMessages.length === 0 ? (
      ...
      chatMessages.map((msg, idx) => (
        <div key={idx} style={{ marginBottom: "10px" }}>
          <strong>{msg.from === "district_collector" ? <i className="fas fa-user"></i> : <i className="fas fa-user-tie"></i>} {msg.from === "district_collector" ? "You" : chatRole}:</strong> {msg.message}
        </div>
      ))
    )}
  </div>
...
export default DistrictCollectorHome;
