/**
 * SurveyorDashboard.js (Final)
 * -----------------------------------
 * File Name: SurveyorDashboard
 * Description: Styled dashboard for Surveyor with 2x2 layout and full theme integration.
...
function SurveyorDashboard() {
  const navigate = useNavigate();
    ...
    // State variables for Rejected Applications
      ...
      fetch(`${backendUrl}/api/applicationdetails/sent?all=true`)
        ...
        .then(data => {
          if (data.applications) {
            // Applications sent to Surveyor
            const sentApps = data.applications.filter(
              app => app.status === "Sent to Surveyor"
            );
            setPendingCount(sentApps.length);
            
            // Applications received in the last 5 days
            const fiveDaysAgo = new Date(Date.now() - 5 * 24 * 60 * 60 * 1000);
            const recentApps = sentApps.filter(
              app => new Date(app.createdtimestamp) >= fiveDaysAgo
            );
            setRecentCount(recentApps.length);
            
            // Rejected applications filtered by "Rejected by Surveyor"
            const rejectedApps = data.applications.filter(
              app => app.status === "Rejected by Surveyor"
            );
            setRejectedCount(rejectedApps.length);
            // Applications rejected in the last 2 days
            const twoDaysAgo = new Date(Date.now() - 2 * 24 * 60 * 60 * 1000);
            const rejectedRecentApps = rejectedApps.filter(
              app => new Date(app.lastmodifiedtimestamp) >= twoDaysAgo
            );
    ...
    // Fetch notes for the logged in official (type "surveyor")
    const fetchUserNotes = () => {
      if (official && official.officialid) {
        fetch(`${backendUrl}/api/notes?type=surveyor&username=${official.officialid}`)
          .then(response => response.json())
          .then(data => {
            if (data.success) {
    ...
    <ThemeProvider theme={theme}>
      <Box sx={{ minHeight: '100vh', background: 'linear-gradient(145deg, #cfd9df, #e2ebf0)', display: 'flex', flexDirection: 'column' }}>
        <Box sx={{ background: 'linear-gradient(to right, #434343, #000000)', color: 'white', p: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <Typography variant="h4" fontWeight={600} sx={{ color: 'white' }}>Surveyor Dashboard</Typography>
        ...
        <Box sx={{ display: 'flex', flex: 1 }}>
          <Box sx={{ width: 80, background: 'linear-gradient(to bottom, #868f96, #596164)', color: 'white', display: 'flex', flexDirection: 'column', alignItems: 'center', py: 3 }}>
            {[{ icon: 'globe', path: '/surveyorhome' }, { icon: 'sign-out-alt', action: handleLogout }].map((item, i) => (
          ...
          <Box flex={1} p={3} display="grid" gridTemplateColumns="repeat(2, 1fr)" gap={3}>
            ...
            <Paper elevation={3} sx={{ p: 2, background: 'linear-gradient(145deg, #cfd9df, #e2ebf0)' }}>
              ...
              <Box display="flex" gap={2} flexWrap="wrap">
                <Paper sx={{ p: 2, flex: 1, minWidth: 180, background: 'linear-gradient(145deg, #cfd9df, #e2ebf0)' }}>
                  <Typography fontWeight={600}>Total Pending</Typography>
                  <Typography variant="h4">{pendingCount}</Typography>
                </Paper>
                <Paper sx={{ p: 2, flex: 1, minWidth: 180, background: 'linear-gradient(145deg, #cfd9df, #e2ebf0)' }}>
                  <Typography fontWeight={600}>Recent (5 Days)</Typography>
            ...
            <Paper elevation={3} sx={{ p: 2, background: 'linear-gradient(145deg, #cfd9df, #e2ebf0)' }}>
              ...
              <Box display="flex" gap={2} flexWrap="wrap">
                <Paper sx={{ p: 2, flex: 1, minWidth: 180, background: 'linear-gradient(145deg, #cfd9df, #e2ebf0)' }}>
                  <Typography fontWeight={600}>Rejected by Surveyor</Typography>
                  <Typography variant="h4">{rejectedCount}</Typography>
                </Paper>
                <Paper sx={{ p: 2, flex: 1, minWidth: 180, background: 'linear-gradient(145deg, #cfd9df, #e2ebf0)' }}>
                  <Typography fontWeight={600}>Rejected in 2 Days</Typography>
