...
function MinistryWelfareHome() {
  ...
  const [chatRole, setChatRole] = useState("District Collector");
  ...
  const fetchPendingApplications = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const pending = (data.applications || []).filter(app => (app.status === "Sent to Ministry of Welfare"));
  ...
  const fetchChatApplications = async () => {
    if (!official?.officialid) return;
    try {
      const response = await fetch(`/api/chats/ministrywelfare`);
      const data = await response.json();
      if (response.ok) {
  ...
  const fetchSentApplications = async () => {
    ...
    try {
      const response = await fetch(`/api/applicationdetails/sent`);
      const data = await response.json();
      if (response.ok) setSentApplications((data.applications || []).filter(app => (app.status !== "Sent to ITDA" && app.status !=="created" && app.status!="Pending with User" && app.status != "Sent to Super Intendent" && app.status != "Rejected by Super Intendent" && app.status != "Sent to Project Officer 1" && app.status != "Rejected by Project Officer" && app.status != "Sent to MRO" && app.status != "Rejected by MRO" && app.status != "Sent to Surveyor" && app.status != "Rejected by Surveyor" && app.status != "Sent to Revenue Inspector" && app.status != "Rejected by Revenue Inspector" && app.status != "Sent to VRO" && app.status != "Rejected by VRO" && app.status != "Sent to RDO" && app.status != "Rejected by RDO" && app.status != "Sent to Joint Collector" && app.status != "Rejected by Joint Collector" && app.status != "Sent to District Collector" && app.status != "Rejected by District Collector" && app.status != "Sent to Ministry of Welfare")));
  ...
  const handleModalAction = async (actionType) => {
    ...
    if (actionType === "proceed") {
      newStatus = "Sent to Project Officer 2";
    } else if (actionType === "deny") {
      if (!denialReason.trim()) {
        alert("Please provide a reason for denial.");
        return;
      }
      newStatus = "Rejected by Ministry of Welfare";
      reason = denialReason;
    }
    try {
  ...
  const loadChatHistory = async (role, app = selectedChatApp) => {
    if (!app) return;
    try {
      const response = await fetch(`/api/chats/mw?appId=${app?._id || ""}&receiptid=${app?.receiptid || ""}&role=${role}`);
      const data = await response.json();
      if (response.ok) {
  ...
  return (
    <div>
      <style dangerouslySetInnerHTML={{ __html: `
        /* Custom Theme and Layout Styles */
body {
...
.button-takeaction {
  background: linear-gradient(135deg,rgb(22, 23, 23),rgb(89, 85, 85));
}
.button-proceed {
...
.modal-input {
  ...
  font-size: 16px;
      ...
      <div className="blue-line">
        ...
        <div className="header-title"> Ministry of Welfare Dashboard</div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: '20px', marginTop: '20px' }}>
                <div style={{
                  ...
                  <p><i className="fas fa-user-tag"></i> Role: Ministry of Welfare</p>
    ...
    {chatMessages.length === 0 ? (
      ...
      chatMessages.map((msg, idx) => (
        <div key={idx} style={{ marginBottom: "10px" }}>
          <strong>{msg.from === "ministry_welfare" ? <i className="fas fa-user"></i> : <i className="fas fa-user-tie"></i>} {msg.from === "ministry_welfare" ? "You" : chatRole}:</strong> {msg.message}
        </div>
      ))
    )}
  </div>
...
export default MinistryWelfareHome;
