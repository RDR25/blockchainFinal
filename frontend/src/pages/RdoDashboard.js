/**
 * RdoDashboard.js (Final)
 * -------------------------
 * File Name: RdoDashboard
 * Description: Styled dashboard for RDO with complete layout, theme, and visual consistency.
 ...
 * -----------------------------------
 * File Name: RdoDashboard
 * Description: Styled dashboard for Project Officer with full theme, layout, and gradient updates.
 * DO NOT MODIFY BACKEND OR STRUCTURE.
 */
/**
 * RdoDashboard.js (Final)
 * -------------------------
 * File Name: RdoDashboard
 * Description: Styled dashboard for MRO with 2x2 layout, correct theme, and role-specific adaptations.
 * DO NOT MODIFY BACKEND OR STRUCTURE.
 */
/**
 * RdoDashboard.js (Final)
...
function RdoDashboard() {
  const navigate = useNavigate();
      ...
      fetch(`${backendUrl}/api/applicationdetails/sent?all=true`)
        ...
        .then(data => {
          if (data.applications) {
            // Applications sent to RDO
            const sentApps = data.applications.filter(app => app.status === "Sent to RDO");
            ...
            // Applications with status "Rejected by RDO"
            const rejectedApps = data.applications.filter(app => app.status === "Rejected by RDO");
    ...
    // Fetch notes for the logged in official (type "revenue_dept_officer")
    const fetchUserNotes = () => {
      if (official && official.officialid) {
        fetch(`${backendUrl}/api/notes?type=revenue_dept_officer&username=${official.officialid}`)
          .then(response => response.json())
          .then(data => {
            if (data.success) {
    ...
    // Add a note with type "revenue_dept_officer"
    const handleAddNote = () => {
    ...
    <ThemeProvider theme={theme}>
      <Box sx={{ minHeight: '100vh', background: 'linear-gradient(145deg, #cfd9df, #e2ebf0)', display: 'flex', flexDirection: 'column' }}>
        <Box sx={{ background: 'linear-gradient(to right, #434343, #000000)', color: 'white', p: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <Typography variant="h4" fontWeight={600} sx={{ color: 'white' }}>RDO Dashboard</Typography>
        ...
        <Box sx={{ display: 'flex', flex: 1 }}>
          <Box sx={{ width: 80, background: 'linear-gradient(to bottom, #868f96, #596164)', color: 'white', display: 'flex', flexDirection: 'column', alignItems: 'center', py: 3 }}>
            {[{ icon: 'globe', path: '/rdohome' }, { icon: 'sign-out-alt', action: handleLogout }].map((item, i) => (
          ...
          <Box flex={1} p={3} display="grid" gridTemplateColumns="repeat(2, 1fr)" gap={3}>
            ...
            <Paper elevation={3} sx={{ p: 2, background: 'linear-gradient(145deg, #cfd9df, #e2ebf0)' }}>
              ...
              <Box display="flex" gap={2} flexWrap="wrap">
                <Paper sx={{ p: 2, flex: 1, minWidth: 180, background: 'linear-gradient(145deg, #cfd9df, #e2ebf0)' }}>
                  <Typography fontWeight={600}>Rejected by RDO</Typography>
                  <Typography variant="h4">{rejectedCount}</Typography>
                </Paper>
                <Paper sx={{ p: 2, flex: 1, minWidth: 180, background: 'linear-gradient(145deg, #cfd9df, #e2ebf0)' }}>
                  <Typography fontWeight={600}>Rejected in last 2 Days (&gt;2 Days)</Typography>
