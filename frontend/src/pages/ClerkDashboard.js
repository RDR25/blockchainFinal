...
/**
 * ClerkDashboard.js (Fixed)
 * ------------------------------
 * File Name: ClerkDashboard
...
function ClerkDashboard() {
    ...
    fetch(`${backendUrl}/api/applicationdetails/sent?all=true`)
      ...
      .then((data) => {
        if (data.applications) {
          const sent = data.applications.filter((a) => a.status === 'Sent to ITDA');
          ...
          setPendingWithUserCount(data.applications.filter((a) => a.status === 'Pending with User').length);
          setPendingWithUserOldCount(data.applications.filter((a) => a.status === 'Pending with User' && new Date(a.lastmodifiedtimestamp) < twoDaysAgo).length);
        }
      })
      .catch(console.error);
  }, [backendUrl]);
  ...
  useEffect(() => {
    if (official?.officialid) {
      fetch(`${backendUrl}/api/notes?type=clerk&username=${official.officialid}`)
        .then((res) => res.json())
  ...
  const handleAddNote = () => {
    ...
    fetch(`${backendUrl}/api/notes`, {
      ...
      body: JSON.stringify({ type: 'clerk', username: official.officialid, notes: noteText })
    })
      .then((res) => res.json())
      .then((data) => {
        if (data.success) {
    ...
    <ThemeProvider theme={theme}>
      <Box sx={{ minHeight: '100vh', background: 'linear-gradient(145deg, #cfd9df, #e2ebf0)', display: 'flex', flexDirection: 'column' }}>
        <Box sx={{ background: 'linear-gradient(to right, #434343, #000000)', color: 'white', p: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <Typography variant="h4" fontWeight={600} sx={{ color: 'white' }}>Clerk Dashboard</Typography>
        ...
        <Box sx={{ display: 'flex', flex: 1 }}>
          <Box sx={{ width: 80, background: 'linear-gradient(to bottom, #868f96, #596164)', color: 'white', display: 'flex', flexDirection: 'column', alignItems: 'center', py: 3 }}>
            {[{ icon: 'globe', path: '/clerkhome' }, { icon: 'sign-out-alt', action: handleLogout }].map((item, i) => (
          ...
          <Box flex={1} p={3} display="grid" gridTemplateColumns="repeat(2, 1fr)" gap={3}>
            <Paper elevation={3} sx={{ p: 2, background: 'linear-gradient(145deg, #cfd9df, #e2ebf0)' }}>
              ...
              <Box display="flex" gap={2} flexWrap="wrap">
                ...
                <Paper sx={{ p: 2, flex: 1, minWidth: 180, background: 'linear-gradient(145deg, #cfd9df, #e2ebf0)' }}>
                  <Typography fontWeight={600}>Recent in last 5 Days</Typography>
            ...
            <Paper elevation={3} sx={{ p: 2, background: 'linear-gradient(145deg, #cfd9df, #e2ebf0)' }}>
              ...
              <Box display="flex" gap={2} flexWrap="wrap">
                <Paper sx={{ p: 2, flex: 1, minWidth: 180, background: 'linear-gradient(145deg, #cfd9df, #e2ebf0)' }}>
                  <Typography fontWeight={600}>Pending with User</Typography>
                  <Typography variant="h4">{pendingWithUserCount}</Typography>
                </Paper>
                <Paper sx={{ p: 2, flex: 1, minWidth: 180, background: 'linear-gradient(145deg, #cfd9df, #e2ebf0)' }}>
                  <Typography fontWeight={600}>Pending with User (&gt;2 Days)</Typography>
                  <Typography variant="h4">{pendingWithUserOldCount}</Typography>
                </Paper>
              </Box>
            </Paper>
